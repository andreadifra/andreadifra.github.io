...
All six `glmmTMB` candidates passed these checks, including the compact ZINB shown above. This result supports using their fitted likelihoods and estimates in the next steps. However, it does not establish that any candidate reproduces the observed count distribution, which we address in the next section with simulation diagnostics.
