...
Count data often looks simple until the zeros start piling up. A standard Poisson or negative binomial model assumes that every observation comes from the same count-generating process. In many applied settings, that assumption is too tidy: some people, places, or systems are never really at risk of the event, while others are at risk but happen to record zero during the observation window.

Zero-inflated models give us a way to represent that distinction explicitly. This tutorial builds up from generalized linear models to zero-inflated Poisson and zero-inflated negative binomial models, using healthcare utilization as the running example.
...
![Zero-inflated models treat observed zeros as a mixture of structural and sampling zeros, while hurdle models put all zeros on one side of a participation hurdle and positive counts on the other.](hurdle-vs-zi.svg){#fig-hurdle-vs-zi fig-alt="A hand-drawn comparison diagram showing observed zeros and positive counts split differently by zero-inflated and hurdle models."}
