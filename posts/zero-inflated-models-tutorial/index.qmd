...
Because NB2 assigns more probability to zero than a Poisson distribution with the same mean, it may account for some of an observed zero surplus without a separate structural-zero process. The later model comparison therefore includes both negative binomial and zero-inflated negative binomial models.
...
First, we collapse the outcome to **zero versus one or more visits** and fit a logistic regression. It's important to remember that this is not the same as the zero component of a fitted zero-inflated model, and it cannot identify which observed zeros are structural. However, it provides a useful comparison ... simply asks whether the adjusted probability of recording no visits varies with the available predictors.
...
Here $X_i$ is the row vector of count-component predictor values for person $i$, and $\beta$ is the corresponding coefficient vector. Likewise, $Z_i$ is the row vector of zero-inflation predictor values, and $\gamma$ is its coefficient vector. The two row vectors may contain different predictors and normally include an intercept unless the formula removes it.
...
**For zeros:**
$$P(Y = 0) = \pi + (1-\pi)\left(\frac{\phi}{\phi+\mu}\right)^\phi$$

**For positive counts:**
$$P(Y = k) = (1-\pi)\frac{\Gamma(k+\phi)}{k!\Gamma(\phi)}\left(\frac{\phi}{\phi+\mu}\right)^\phi\left(\frac{\mu}{\phi+\mu}\right)^k$$

The difference from ZIP is practical:

- Uses negative binomial instead of Poisson for the count component
- Parameter $\phi$ controls additional overdispersion within the potential user group
- As $\phi \to \infty$, the negative binomial count distribution approaches a Poisson distribution

The same distinction between conditional and population moments applies here. Under the `nbinom2` parameterization used later, the count process has mean $\mu$ and variance $\mu + \mu^2/\phi$. After mixing in the extra-zero group,

$$
\begin{aligned}
E[Y] &= (1-\pi)\mu, \\
\operatorname{Var}(Y)
  &= \underbrace{(1-\pi)\left(\mu + \frac{\mu^2}{\phi}\right)}_{
...
ZINB therefore adds one source of variation to the ZIP model: the $\mu^2/\phi$ term allows overdispersion within the count-generating group, while the final term still captures variation from mixing that group with an extra-zero group. A second worked example would repeat the ZIP calculation without adding much intuition, so I will use this decomposition as the bridge to the fitted models.
...
zinb_spec_models <- list(
  ...
  `Need/access proxy set` = glmmTMB(
    count_formula,
    ziformula = ~ healthexcellent + age + male,
    data = medcare,
    family = nbinom2()
  ),
  `Shared adjustment set` = zinb_model
)
...
zinb_spec_table <- tibble::tibble(
  ...
  Rationale = c(
    ...
    "Adds basic demographic proxies without sharing the full count formula.",
    "Lets each baseline predictor have a distinct count and zero-process role."
  ),
...
The shared adjustment set is the working specification for the rest of the article. It has the best AIC and BIC among this small set, and it is interpretable: chronic conditions, health status, age, sex, and marital status can plausibly affect visit intensity and the probability of being in the latent extra-zero group. That does not mean every coefficient will be precise or causally meaningful. It means the formula is a defensible working model for this tutorial.
