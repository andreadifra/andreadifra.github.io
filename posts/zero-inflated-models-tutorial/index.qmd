...
The rest of the tutorial uses a deliberate sequence. We start with Poisson regression, check how it fails, move to negative binomial regression when the count process is overdispersed, then ask whether a separate zero-inflation process is still useful.

### Poisson Regression

Poisson regression is the standard first model for a count outcome. For person $i$, it assumes

$$Y_i \mid X_i \sim \operatorname{Poisson}(\lambda_i), \qquad \log(\lambda_i) = X_i\beta.$$

The log link keeps the expected count $\lambda_i$ positive, and the exponentiated coefficients are rate ratios. If a coefficient is $0.20$, the corresponding rate ratio is $e^{0.20}=1.22$, meaning a 22% higher expected count for a one-unit increase in that predictor, holding the other predictors fixed.

The load-bearing assumption is **conditional equidispersion**:

$$E(Y_i \mid X_i) = \operatorname{Var}(Y_i \mid X_i) = \lambda_i.$$

The word "conditional" matters. A raw variance-to-mean ratio from the outcome can warn us that Poisson regression may struggle, but the model assumption is about the distribution after conditioning on the predictors in the formula.

### Overdispersion and Negative Binomial Regression

Overdispersion occurs when the fitted model allows too little conditional variation. In count data this often happens because people with the same observed predictors still differ in unobserved health needs, access, preferences, or event risk. A Poisson model then underestimates uncertainty and may also underpredict zeros and large counts.

Negative binomial regression is the natural next one-process candidate. It keeps the same log mean model,

$$\log(\mu_i) = X_i\beta,$$

but relaxes the Poisson variance assumption. The `nbinom2` family used by `glmmTMB` has the quadratic variance function

$$\operatorname{Var}(Y_i \mid X_i) = \mu_i + \frac{\mu_i^2}{\phi}.$$

Here $\phi$ is the dispersion parameter: larger values move the model closer to Poisson, while smaller values allow more extra-Poisson variation. This matters for zero-inflated modelling because an overdispersed one-process distribution can generate more zeros than a Poisson distribution with the same mean. Negative binomial regression is therefore a serious competitor, not just a stepping stone to ZINB.

::: {.callout-note appearance="simple" collapse="true"}
## Advanced aside: generalized Poisson models

<!-- TODO: Should include brief formulas to show how this changes i.e for example the variance formula and how this is different from NB -->

Poisson and negative binomial models are not the only count-process choices. A generalized Poisson model is another way to relax the Poisson mean-variance relationship, and it can handle some forms of overdispersion or underdispersion. I do not fit one in this tutorial because the main goal is to understand zero-inflated Poisson and negative binomial models, but it is a useful nearby option when the negative binomial variance pattern is a poor match.
:::
