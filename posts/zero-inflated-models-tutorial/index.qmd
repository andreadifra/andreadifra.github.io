...
Unlike the negative binomial baseline, which keeps every participant in one count process, a zero-inflated model assumes that the data arise from two connected processes: one that produces structural zeros and another that produces counts, including sampling zeros.
...
The same distinction between conditional and population moments applies here. Under the `nbinom2` parameterization used for the baseline and ZINB models, the count process has mean $\mu$ and variance $\mu + \mu^2/\phi$. After mixing in the extra-zero group,
