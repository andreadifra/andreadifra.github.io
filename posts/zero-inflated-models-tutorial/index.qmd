...
The exploratory logistic regression in @tbl-logistic-analysis found the clearest adjusted associations with an observed zero for chronic conditions, sex, and marital status. I therefore carry those variables forward as a compact starting formula for the zero-inflation component. This choice is empirically informed, but it does not identify the true extra-zero process: the earlier regression combines sampling zeros and latent extra zeros in a single observed-zero outcome.

Health status, chronic conditions, age, sex, and marital status enter the count component. The dataset lacks direct measures of access and eligibility, so neither component should be read as a complete mechanism.
...
The Poisson model and `count_formula` were created in the baseline section, so we reuse them rather than fitting the same model twice. For the first comparison, ZIP and ZINB also share the compact zero-inflation formula motivated above. Holding both formulas fixed isolates the main family contrast: Poisson versus negative-binomial variation in the count process. After that comparison, we revisit the zero-inflation formula as a specification decision.
...
zip_model <- glmmTMB(
  count_formula,
  ziformula = baseline_zi_formula, # <4>
  data = medcare,
  family = poisson()
)
...
3. The negative binomial model keeps one count-generating process and adds conditional overdispersion.
4. ZIP and ZINB use the same count and zero-inflation formulas, so their difference in this comparison is the count distribution.
