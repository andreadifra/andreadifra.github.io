...
description: "From excess zeros to a defensible model: a practical guide to testing, diagnosing, and interpreting zero inflation in R."
image: figures/publication-image.png
...
title-block-banner: figures/publication-image.png
open-graph: true
twitter-card:
...
1. A latent extra-zero state, whose zeros are often called **structural zeros**.
2. A count-generating state, which produces **sampling zeros** as well as positive counts.
...
![Healthcare zeros can be interpreted through two possible latent routes: an extra-zero state associated with policy or access barriers, and ordinary non-use during the observation window.](figures/zero-sources.svg){#fig-zero-sources fig-alt="A hand-drawn flow diagram showing two possible routes to a healthcare count of zero: an extra-zero state associated with access or eligibility barriers, and ordinary non-use among people who could have attended."}
...
![A generalized linear model turns predictors into a linear predictor, transforms it through a link function, and uses the result as the expected value for an observation model.](figures/glm-pipeline.png){#fig-glm-pipeline fig-alt="A hand-drawn left-to-right pipeline: predictors, linear predictor, link function, expected response, and observed outcome distribution."}
...
We use 500 replicates to keep the tutorial practical to reproduce. Substantive work should use more replicates when necessary and check that the interval endpoints are stable. The bootstrap used `r n_workers` local workers and successfully refitted `r format(n_boot_success, big.mark = ",")` of `r format(n_boot, big.mark = ",")` simulated datasets. The interval calculation excludes any refit that fails the convergence and Hessian checks used for the working model; none failed here.
