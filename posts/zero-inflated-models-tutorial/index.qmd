...
## Mixture Probabilities and Joint Likelihood

The two latent states create two routes to an observed zero but only one route to a positive count. Let $f_c(k;\mu_i,\psi)$ denote the probability that the count component assigns to the value $k$.

- If $y_i=0$, the zero may come directly from the extra-zero state, with probability $\pi_i$, or from the count-generating state producing zero, with probability $(1-\pi_i)f_c(0;\mu_i,\psi)$.
- If $y_i>0$, the observation must come from the count-generating state, with probability $(1-\pi_i)f_c(y_i;\mu_i,\psi)$.
...
This equation does not classify a zero as one type or the other. Instead, it adds the probabilities of both possible routes.

*If your main goal is to fit and interpret zero-inflated models, you can skip the next equation. The paragraph after it contains the practical point.*

To fit the model, we combine the likelihood contributions from all $n$ participants. The parameters $\beta$ and $\gamma$ are the count- and zero-inflation coefficient vectors introduced more fully in Section 4.4:

$$
L(\beta,\gamma,\psi)
=\prod_{i=1}^{n}L_i\bigl(\pi_i(\gamma),\mu_i(\beta),\psi\bigr).
$$

The product tells the fitting procedure to find one set of count, zero-inflation, and dispersion parameters that collectively makes the observed data most plausible. Both components are therefore estimated jointly. The model does not first classify the zeros and then fit two separate regressions.

This distinction also explains why the preliminary logistic regression in @tbl-logistic-analysis is not the fitted zero-inflation component. That regression predicts whether an observed count equals zero, regardless of which state produced it. The fitted zero-inflation component instead models the unobserved event $S_i=1$. Let $p_{0i}=P(Y_i=0\mid X_i,Z_i)$ denote the overall probability of zero. The two outcomes have different probabilities:

$$
\begin{aligned}
p_{0i} &=\pi_i \\
  &\quad +(1-\pi_i)f_c(0;\mu_i,\psi), \\
P(S_i=1\mid Z_i) &=\pi_i.
\end{aligned}
$$

Although the state remains unobserved, seeing a zero allows the model to update the probability that the extra-zero state generated it. Bayes' rule gives

$$
\begin{aligned}
&P(S_i=1\mid Y_i=0,X_i,Z_i) \\
&\qquad=\frac{\pi_i}{p_{0i}}.
\end{aligned}
$$

The numerator is the probability of the extra-zero route. The denominator is the total probability of observing zero through either route. For a positive count, the posterior probability of the extra-zero state is zero because that state cannot produce a positive value.

For a concrete ZIP example, suppose the model assigns observations with a particular covariate profile $\pi_i=0.15$ and a Poisson count-component mean $\lambda_i=3.2$.
...
The model therefore assigns the zero more strongly to the extra-zero route, but it does not establish the state with certainty or turn it into an observed diagnosis.

## Conditional Mixture Mean and Variance

The mean and variance of a zero-inflated outcome combine the two latent states. Suppose the count-generating state has mean $\mu_i$ and variance $V_c(\mu_i)$, where $V_c$ is the variance function of the selected count distribution:
...
The extra-zero state contributes a value of zero with probability $\pi_i$, while the count-generating state contributes an average of $\mu_i$ with probability $1-\pi_i$. Averaging over those states gives
...
This is the **conditional mixture mean**: it is conditional on the observed predictors $X_i$ and $Z_i$, but averages over the unobserved state $S_i$. It is smaller than the count-component mean $\mu_i$ whenever $\pi_i>0$.

The mixture variance has two sources: variation among counts within the count-generating state, and additional variation from mixing that state with an always-zero state.

*The variance derivation is not required for fitting or interpreting the models. You can skip to the table below; the key point is that the two sources of variation are added together.*

Applying the law of total variance gives the two-term decomposition

$$
\begin{aligned}
\operatorname{Var}(Y_i\mid X_i,Z_i)
&=(1-\pi_i)V_c(\mu_i) \\
&\quad+\pi_i(1-\pi_i)\mu_i^2.
\end{aligned}
$$

In the final line, $(1-\pi_i)V_c(\mu_i)$ is the contribution from variation within the count-generating state. The term $\pi_i(1-\pi_i)\mu_i^2$ is the additional variation created by mixing the two states.

We can now substitute the variance functions from Section 2 rather than derive ZIP and ZINB separately. The final column of @tbl-zero-inflated-moments shows the resulting variance of the observed mixture; the preceding columns identify the count-component ingredients.
...
Both models have conditional mixture mean $(1-\pi_i)\mu_i$. ZIP combines an extra-zero state with a Poisson count state. ZINB adds the term $\mu_i^2/\phi$, allowing overdispersion within the count-generating state as well. As $\phi\to\infty$, that additional NB2 variation disappears and the ZINB expressions approach their ZIP counterparts.

The practical summary is that zero inflation changes both the mean and the variance. It reduces the overall expected count by the factor $1-\pi_i$ and adds variation because observations may arise from different latent states.

## Regression Structure and Overall Expected Counts

The model needs one regression for each component. Let $X_i$ contain the count-component predictors and $\beta$ their coefficients. Let $Z_i$ contain the zero-inflation predictors and $\gamma$ their coefficients. The two regressions are
...
The log link keeps the count-component mean $\mu_i$ positive. The logit link keeps the extra-zero probability $\pi_i$ between zero and one. The two components may use different predictors, although both normally include an intercept.

The coefficients answer component-specific questions:

- $e^{\beta_j}$ is a rate ratio within the count-generating state.
- $e^{\gamma_j}$ is an odds ratio for membership in the extra-zero state.

Neither quantity automatically describes the predictor's combined association with observed visits. The overall expected count combines both components:

$$
m_i=E(Y_i\mid X_i,Z_i)=(1-\pi_i)\mu_i.
$$

For example, a predictor may increase the expected number of visits within the count-generating state while also increasing the probability of the extra-zero state. The first pathway raises the overall mean, while the second lowers it.

*The next equation makes this combined effect explicit, but it is optional for following the case study. Section 5 uses response-scale predictions to provide the practical interpretation.*

For the additive linear predictors used here, if $x_j$ appears in both components, a one-unit increase gives the overall expected-count ratio
...
The first factor is the change in the count-component mean. The second is the change in the probability of being in the count-generating state. Because the second factor depends on the covariate profile, the combined ratio can also vary across profiles.

If $x_j$ appears only in the count component, $\pi_i$ remains fixed and the overall ratio reduces to $e^{\beta_j}$. If it appears only in the zero-inflation component, the first factor equals one and the overall association operates through $\pi_i$. Chronic conditions, sex, and marital status enter both components of the working model, so Section 5 supplements their component coefficients with response-scale predictions.
