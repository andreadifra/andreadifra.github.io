...
Unmodelled heterogeneity, an inadequate mean specification, or overdispersion in the count process could also produce more zeros than a Poisson model predicts. Negative binomial and zero-inflated models therefore remain competing explanations, and we explore them in the next section.

# How Zero-Inflated Models Work

## The Core Concept: Two-Process Thinking

The Poisson checks justify considering more flexible models: we introduce the zero-inflated candidate next because its two-process structure requires explanation before it can be compared with the one-process negative binomial alternative. The model assumes that the data arise from two connected processes: one that produces structural zeros and another that produces counts, including sampling zeros.
